...
const ApproveList = () => {
  ...
  const unverifiedDonors = useSelector(state => state.directorReducer.unverifiedDonors);
  ...
  const [pageDonor, setPageDonor] = useState(0);
  ...
  const onChangePageDonor = async (idx) => {
    setPageDonor(idx);
    await dispatch(retrieveUnverifiedUsers(
      {
        limit: APPROVE_COUNT,
